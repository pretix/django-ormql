...
class NumericResolveMixin:
    ...
    def _resolve_output_field(self):
        ...
        source_types = set(type(source) for source in self.get_source_fields() if source is not None)
        if len(source_types) == 1:
            return list(source_types)[0]()
        elif source_types == {fields.DecimalField, fields.IntegerField}:
            return fields.DecimalField(
                max_digits=max(f.max_digits for f in self.get_source_fields() if isinstance(f, fields.DecimalField)),
                decimal_places=max(f.decimal_places for f in self.get_source_fields() if isinstance(f, fields.DecimalField)),
            )
            ...
            return fields.DecimalField(
                max_digits=max(f.max_digits for f in self.get_source_fields() if isinstance(f, fields.DecimalField)),
                decimal_places=max(
                    f.decimal_places for f in self.get_source_fields() if isinstance(f, fields.DecimalField)
                ),
            )
        elif source_types == {fields.FloatField, fields.DecimalField, fields.IntegerField}:
            return fields.DecimalField(
                max_digits=max(f.max_digits for f in self.get_source_fields() if isinstance(f, fields.DecimalField)),
                decimal_places=max(
                    f.decimal_places for f in self.get_source_fields() if isinstance(f, fields.DecimalField)
...
class Div(NumericResolveMixin, Func):
    ...
    def __init__(self, *expressions, output_field=None, **extra):
        ...
        super().__init__(
            expressions[0],
            ExpressionWrapper(expressions[1] * Value(1.0), output_field=fields.FloatField()),
