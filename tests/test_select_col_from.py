...
def test_case_sensitive(engine_t1):
    with pytest.raises(
        QueryError, match="Column 'caTegory' does not exist in table 'products'"
    ):
        list(
            engine_t1.query(
                """
...
def test_multiple_tables_not_allowed(engine_t1):
    with pytest.raises(
        QueryNotSupported, match="SELECT from multiple tables not supported"
    ):
        list(
            engine_t1.query(
                """
...
def test_unknown_field(engine_t1):
    with pytest.raises(
        QueryError, match="Column 'name' does not exist in table 'categories'"
    ):
        list(
            engine_t1.query(
                """
